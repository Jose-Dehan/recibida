...
export default function ReservationPage({ params }: { params: Promise<{ codigo: string }> }) {
  ...
  return <MobilePageContainer>
    ...
    <p className={`mt-4 rounded-[20px] border p-4 text-sm ${data.emailSent !== false ? "border-emerald-400/20 bg-emerald-400/[0.07] text-emerald-200" : "border-amber-400/20 bg-amber-400/[0.07] text-amber-200"}`}>{data.emailSent !== false ? "Te enviamos el código de reserva a tu email." : "No pudimos enviar el email. Guardá este código de reserva."}</p>
