...
export function ConfirmationBottomSheet({ open, values, price, onClose }: { open: boolean; values: ReservationFormValues; price: number; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 p-0 sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="confirmation-title" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <section className="max-h-[92vh] w-full max-w-md overflow-y-auto rounded-t-3xl border border-line bg-panel p-6 shadow-sheet sm:rounded-3xl">
        <div className="mx-auto mb-5 h-1 w-12 rounded-full bg-zinc-600 sm:hidden" />
        <p className="eyebrow">Último paso</p>
        <h2 id="confirmation-title" className="mt-2 text-2xl font-black">Confirmar reserva</h2>
        <dl className="mt-6 divide-y divide-line rounded-xl border border-line px-4">
          {[["Nombre", values.name], ["DNI", values.dni], ["Email", values.email], ["Precio", formatPrice(price)]].map(([label, value]) => (
        ...
        <div className="mt-6 space-y-3">
          <PrimaryButton type="button" onClick={() => router.push(`/reserva/${mockReservationCode}`)}>Confirmar reserva</PrimaryButton>
